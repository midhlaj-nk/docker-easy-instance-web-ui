    ...
    (set, get) => ({
      ...
      getCurrentUser: async () => {
        ...
        try {
          ...
          // If it's a 401 or auth error, logout
          if (error.message.includes('expired') || error.message.includes('unauthorized') || error.message.includes('401')) {
            get().logout();
          }
  ...
  addCollaborator: async (instanceId, username, permissions) => {
    ...
    try {
      ...
      if (response.ok && (data.success || data.data?.success)) {
        set({ isLoading: false });
        return { success: true, data: data.data || data, msg: data.message || data.data?.msg };
      } else {
        set({ error: data.message || data.data?.msg || 'Failed to add collaborator', isLoading: false });
        return { success: false, error: data.message || data.data?.msg || 'Failed to add collaborator', data };
      }
    } catch (error) {
      set({ error: error.message, isLoading: false });
      return { success: false, error: error.message };
  ...
  removeCollaborator: async (instanceId, username) => {
    ...
    try {
      ...
      if (response.ok && (data.success || data.data?.success)) {
        ...
        return { success: false, error: data.message || 'Failed to remove collaborator', data };
      }
    } catch (error) {
      set({ error: error.message, isLoading: false });
      return { success: false, error: error.message };
  ...
  removeInvitation: async (instanceId, username, invitationId) => {
    ...
    try {
      ...
      if (response.ok && (data.success || data.data?.success)) {
        ...
        return { success: false, error: data.message || 'Failed to remove invitation', data };
      }
    } catch (error) {
      set({ error: error.message, isLoading: false });
      return { success: false, error: error.message };
